...
def get_args():
    ...
    parser.add_argument('--exploration', type=str, default='re3')
    parser.add_argument('--env-id', type=str, default='AntBulletEnv-v0')
    parser.add_argument('--total-time-steps', type=int, default=200000)
...
if __name__ == '__main__':
    ...
    if args.exploration == 'rise':
        rise = RISE(obs_shape=envs.observation_space.shape, action_shape=envs.action_space.shape, device=device, latent_dim=128, beta=1e-2, kappa=1e-5)
    if args.exploration == 'ride':
        ride = RIDE(obs_shape=envs.observation_space.shape, action_shape=envs.action_space.shape, device=device, latent_dim=128, beta=1e-2, kappa=1e-5)
    if args.exploration == 'rnd':
        rnd = RND(obs_shape=envs.observation_space.shape, action_shape=envs.action_space.shape, device=device,
                    latent_dim=128,lr=0.00025,batch_size=64, beta=1e-2, kappa=1e-5)
    if args.exploration == 'ngu':
        ngu = NGU(envs,
        device=device,
        latent_dim=128,
        lr=0.00025,
    ...
    for i in range(num_episodes):
        ...
        if args.exploration == 'rise':
            intrinsic_rewards = rise.compute_irs(
                rollouts={'observations': model.rollout_buffer.observations},
                time_steps=i * args.n_steps * args.n_envs,
                k=3)
            print("rise内在奖励",intrinsic_rewards)
        if args.exploration == 'ride':
            intrinsic_rewards = ride.compute_irs(
                rollouts={'observations': model.rollout_buffer.observations},
                time_steps=i * args.n_steps * args.n_envs)
            print("ride内在奖励", intrinsic_rewards)
        if args.exploration == 'ngu':
            intrinsic_rewards = ngu.compute_irs(
                rollouts={'observations': model.rollout_buffer.observations},
                time_steps=i * args.n_steps * args.n_envs)
            print("ngu内在奖励", intrinsic_rewards)
        if args.exploration == 'rnd':
            intrinsic_rewards = rnd.compute_irs(
                rollouts={'observations': model.rollout_buffer.observations},
                time_steps=i * args.n_steps * args.n_envs)
            #print("外在奖励", model.rollout_buffer.rewards)
            model.rollout_buffer.rewards += intrinsic_rewards[:, :, 0]
