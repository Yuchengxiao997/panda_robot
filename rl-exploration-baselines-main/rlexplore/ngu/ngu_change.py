...
class NGU(object):
    def __init__(self,
        ...
        if len(self.ob_shape) == 3:
            self.predictor_network = CnnEncoder(self.ob_shape, latent_dim)
            self.target_network = CnnEncoder(self.ob_shape, latent_dim)
        else:
            self.predictor_network = MlpEncoder(self.ob_shape, latent_dim)
            self.target_network = MlpEncoder(self.ob_shape, latent_dim)
    ...
    def compute_irs(self, o,r, time_steps):
        ...
        with torch.no_grad():
            ...
            if len(self.ob_shape) == 3:
                encoded_obs = self.embedding_network(obs[:])
            else:
                encoded_obs = obs[:]

            episodic_rewards = self.pseudo_counts(encoded_obs)
            intrinsic_rewards[:-1, 0] = episodic_rewards[:-1] * life_long_rewards
