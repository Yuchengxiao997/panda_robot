...
class KukaReachEnv(gym.Env):
    ...
    def __init__(self, is_render=False, is_good_view=False):
        ...
        self.y_low_action = -3.14
        self.y_high_action = 3.14
        self.z_low_action = -3.14
        self.z_high_action = 3.14
        self.temp = 0
        ...
        self.action_space = spaces.Box(low=np.array(
            [self.x_low_action, self.y_low_action, self.z_low_action,self.x_low_action, self.y_low_action, self.z_low_action,self.z_low_action]),
                                       high=np.array([
                                           self.x_high_action,
                                           self.y_high_action,
                                           self.z_high_action,
    ...
    def reset(self):
        ...
        return np.array(robot_joint_state).astype(np.float32)
    ...
    def step(self, action):
        ...
        for i in range(7):
            robot_joint_state.append(p.getJointState(self.kuka_id, i)[0])
        robot_joint_state = list(robot_joint_state)
        dv = 0.5
        d0 = action[0] * dv*0
        d1 = action[1] * dv*0
        d2 = action[2] * dv*0
