...
class KukaReachEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.object_id = p.loadURDF(os.path.join(self.urdf_root_path,
                                    ...
                                    basePosition=ya,useFixedBase=True)
        self.object_id2 = p.loadURDF(os.path.join(self.urdf_root_path,
                                                 "random_urdfs/000/000.urdf"),
                                    basePosition=[ya[0],ya[1],
                                        0.35
                                    ], useFixedBase=True)
    ...
    def _reward(self):
        ...
        square_col = (self.robot_state[2] - 0.35) ** 2
        #用机械臂末端和物体的距离作为奖励函数的依据
        self.distance = sqrt(square_dx + square_dy + square_dz)
        self.col = sqrt(square_dx + square_dy + square_col)
        #print(self.distance)
