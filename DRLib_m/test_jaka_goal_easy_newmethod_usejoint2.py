...
class KukaReachEnv(gym.Env):
    ...
    def reset(self):
        ...
        while True:
            ...
            joint1 = random.uniform(-1, 1)
            joint2 = random.uniform(-1, 1)
            joint3 = random.uniform(-1, 1)
            joint4 = random.uniform(-1, 1)
            self.orientation = [joint1, joint2, joint3, joint4]
            self.goal_joint_positions = p.calculateInverseKinematics(
                bodyUniqueId=self.kuka_id,
                endEffectorLinkIndex=self.num_joints - 1,
                targetPosition=self.object_pos,
            ...
            for i in range(self.num_joints):
                p.resetJointState(
                    ...
                    jointIndex=i,
                    targetValue=self.goal_joint_positions[i],
                )
            p.stepSimulation()
            obstacle_distance = self.find_collision(self.pos_list)
            self.current_pos = p.getLinkState(self.kuka_id, self.num_joints - 1)[4]
            self.distance = self.cal_dis(self.current_pos, self.object_pos)
            if (obstacle_distance == 0) and (self.distance < 0.05):
                #print("what we need", temp_flag)
                break
            if temp_flag > 1000:
                break
        self.dd = []
        ...
        p.stepSimulation()
        self.rebuild = {'observation': np.array(self.dd), 'achieved_goal': np.array(self.init_joint_positions),
                   'desired_goal': np.array(self.goal_joint_positions)}
        return self.rebuild
    ...
    def step(self, action):
        ...
        self.pass_joint_positions = robot_joint_state
        ...
        while True:
            joint_positions = self.find_midle()
    ...
    def _reward(self):
        ...
        rebuild = {'observation': np.array(self.observation), 'achieved_goal': np.array(robot_joint_state),
                   'desired_goal': np.array(self.goal_joint_positions)}
        #print("reward",reward)
    ...
    def find_midle(self):
        while True:
            ...
            now_obs = self.forward_step(joint_positions)
            pass_obs = self.forward_step(self.pass_joint_positions)
            distance1 = self.cal_dis(now_obs, self.object_pos)
            self.distance = distance1
            distance2 = self.cal_dis(now_obs, pass_obs)
            midle = []
            midle.append((pass_obs[0] + self.object_pos[0]) / 2)
            midle.append((pass_obs[1] + self.object_pos[1]) / 2)
            midle.append((pass_obs[2] + self.object_pos[2]) / 2)
            self.distance3 = self.cal_dis(now_obs, midle)
            if abs(distance2-distance1)<0.05:
                if self.distance3<0.1:
                    break
