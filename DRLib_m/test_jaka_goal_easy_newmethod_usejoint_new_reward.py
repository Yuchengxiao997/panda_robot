...
class KukaReachEnv(gym.Env):
    ...
    def reset(self):
        ...
        for i in range(0):
            self.a.append([0,0,1])
        self.a = [[0.5281180585653241, 0.030778454992689885, 0.33283001848519617], [0.699409031161341, -0.1871434288005503, 0.6213540256172031], [0.6535251996415457, 0.15225301365168925, 0.4970309760345363], [0.586297940740203, -0.11728033066459331, 0.2599058055081895], [0.521991196798249, -0.1123473840575882, 0.47259181286751195]]
        ...
        joint3 = random.uniform(-3.14, 0)
        joint4 = random.uniform(-3.14, 3.14)
        joint5 = random.uniform(-3.14, 3.14)
        joint6 = random.uniform(-3.14, 3.14)
        #self.init_joint_positions = [0, joint2, joint3, joint4, joint5, joint6]
        #self.init_joint_positions = [0, 2, joint3, joint4, joint5, 0]
        #[0.57, 2, -2, -1.57, -1.57, 0]
        ...
        temp_t = p.getLinkState(self.kuka_id, 5)[4]
        ...
        self.rebuild = {'observation': np.array(self.dd),
    ...
    def _reward(self):
        ...
        midle = []
        self.stack.append([self.robot_state,self.object_state])
        self.stack.append([self.robot_state,self.current_pos1])
        ...
        #将观察信息缩减为只有起始位置和目标位置以及环境的信息
        temp_t = self.robot_state
        self.dd[0] = temp_t[0]
        self.dd[1] = temp_t[1]
        self.dd[2] = temp_t[2]

        real_obs = self.dd
        rebuild = {'observation': np.array(real_obs),
