...
class TD3Torch(OffPolicy):
    ...
    def learn(self, batch_size=100,
        ...
        if self.curios_cal==1000000:
        #if self.curios_cal == 0:
            self.icm = ICM(
                obs_shape=obs_shape,
                act_shape=act_shape,
                device=device,
            ...
            self.re3 = RE3(obs_shape=obs_shape, device=device, latent_dim=128, beta=50, kappa=1e-5)
            self.revd = REVD(obs_shape=obs_shape, device=device, latent_dim=128, beta=0.5, kappa=1e-5)

        #add intrinsic reward
        self.curios_cal = self.curios_cal+1
        #250epoch开始引入好奇心
        if self.curios_cal>1000000:
        #if self.curios_cal >0:
            self.use_curios = 1
            #print("start use curios->>>>>")
        if self.use_curios==1:
            ''' begin ->>>>'''


            intrinsic_rewards = self.icm.compute_irs(o,a,r,time_steps=batch_size+self.curios_flag)
            #intrinsic_rewards = self.re3.compute_irs(o, r, time_steps=batch_size + self.curios_flag)
            #intrinsic_rewards = self.revd.compute_irs(o, r, time_steps=batch_size + self.curios_flag)
            #intrinsic_rewards = self.ngu.compute_irs(o, r, time_steps=batch_size + self.curios_flag)
            #intrinsic_rewards = self.rnd.compute_irs(o, r, time_steps=batch_size + self.curios_flag)
            self.curios_flag += 0.1
            temp_reward = torch.from_numpy(intrinsic_rewards)
            if self.curios_cal%400==0:
                print("内在奖励是什么",intrinsic_rewards[0])
            another = data
            # data['rew'] += temp_reward.to(device)
            # print('real reward:',data['rew'])
            another['rew'] += temp_reward.to(device)
            loss_q, loss_info = self.compute_loss_q(another)
        else:
        ...
        if self.learn_step % self.policy_delay == 0:
            ...
            if self.use_curios==1:
                loss_pi = self.compute_loss_pi(another)
            else:
