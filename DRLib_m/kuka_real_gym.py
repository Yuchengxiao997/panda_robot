...
class KukaReachEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.flag = 0
        pp = [random.uniform(self.x_low_obs,self.x_high_obs),random.uniform(self.y_low_obs,self.y_high_obs),random.uniform(0.3,0.01)]
        self.set_robot(pp)
        self.current_pos = p.getLinkState(self.kuka_id, self.num_joints - 1)[4]
        #print("the end>>>>>>",self.current_pos)
    ...
    def _reward(self):
        ...
        square_dx = (self.robot_state[0] - self.current_pos[0]) ** 2
        square_dy = (self.robot_state[1] - self.current_pos[1]) ** 2
        square_dz = (self.robot_state[2] - self.current_pos[2]) ** 2
        ...
        if abs(self.distance-self.distance2) < 0.01:
